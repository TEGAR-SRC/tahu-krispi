...
func (s *Service) ChangePassword(ctx context.Context, in ChangePasswordInput) error {
	...
	if err != nil || !ok {
		return apperrors.New(apperrors.CodeInvalidCredentials, "current password incorrect")
	}
	if in.Current == in.New {
		return apperrors.New(apperrors.CodeValidation, "new password must differ from current password")
	}
	if len(in.New) < 10 {
...
func (s *Service) VerifyEmail(ctx context.Context, token string) error {
	...
	userID, err := uuid.Parse(val)
	if err != nil {
		return apperrors.New(apperrors.CodeValidation, "verification token invalid")
	}
