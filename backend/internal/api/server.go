...
func (s *Server) registerRoutes() {
	...
	admin.Post("/users/:user_id/attach-org", s.requireStaff(""), s.adminAttachUserToOrg)
	...
	admin.Put("/organizations/:org_id/provider-account", s.requireStaff(""), s.adminUpsertProviderAccount)
	...
	// ---- Per-provider: 4 group baru, jangan universal ----
	// Proxmox: /admin/proxmox/:id/* — murni proxmox (kind==proxmox), guard di proxmoxAdapterFor.
	// GET = infra (NOC readable), POST/PUT/DELETE = platform_admin only.
	proxmoxAdmin := admin.Group("/proxmox/:id")
	proxmoxAdmin.Get("/cluster", s.requireStaff("infra"), s.adminProviderCluster)
	proxmoxAdmin.Get("/containers", s.requireStaff("infra"), s.adminProviderContainers)
	proxmoxAdmin.Get("/nodes/:node/storages", s.requireStaff("infra"), s.adminProviderNodeStorages)
	proxmoxAdmin.Get("/nodes/:node/tasks", s.requireStaff("infra"), s.adminProviderNodeTasks)
	proxmoxAdmin.Get("/nodes/:node/detail", s.requireStaff("infra"), s.adminNodeDetail)
	proxmoxAdmin.Get("/nodes/:node/disks", s.requireStaff("infra"), s.adminNodeDisks)
	proxmoxAdmin.Get("/nodes/:node/certs", s.requireStaff("infra"), s.adminNodeCertificates)
	proxmoxAdmin.Post("/nodes/:node/certs", s.requireStaff(""), s.adminNodeCertificateUpload)
	proxmoxAdmin.Delete("/nodes/:node/certs", s.requireStaff(""), s.adminNodeCertificateDelete)
	proxmoxAdmin.Post("/nodes/:node/command", s.requireStaff(""), s.adminNodeCommand)
	proxmoxAdmin.Post("/nodes/:node/backup", s.requireStaff(""), s.adminNodeBackup)
	proxmoxAdmin.Get("/storages/:storage/content", s.requireStaff("infra"), s.adminStorageContentList)
	proxmoxAdmin.Delete("/storages/:storage/content", s.requireStaff(""), s.adminDeleteStorageContent)
	proxmoxAdmin.Get("/storages/:storage/file-restore", s.requireStaff("infra"), s.adminBackupFileRestoreList)
	proxmoxAdmin.Get("/backup-jobs", s.requireStaff("infra"), s.adminListBackupJobs)
	proxmoxAdmin.Post("/backup-jobs", s.requireStaff(""), s.adminCreateBackupJob)
	...
	proxmoxAdmin.Get("/ha-resources/:sid/migrate", s.requireStaff("infra"), s.adminProxmoxHaMigrateStatus)
	proxmoxAdmin.Post("/ha-resources/:sid/migrate", s.requireStaff(""), s.adminProxmoxHaMigrate)
	...
	proxmoxAdmin.Put("/ha/groups/:group", s.requireStaff(""), s.adminUpdateHAGroup)
	proxmoxAdmin.Delete("/ha/groups/:group", s.requireStaff(""), s.adminDeleteHAGroup)
	proxmoxAdmin.Get("/ha/rules", s.requireStaff("infra"), s.adminListHARules)
	proxmoxAdmin.Post("/ha/rules", s.requireStaff(""), s.adminCreateHARule)
	proxmoxAdmin.Put("/ha/rules/:rule", s.requireStaff(""), s.adminUpdateHARule)
	proxmoxAdmin.Delete("/ha/rules/:rule", s.requireStaff(""), s.adminDeleteHARule)
	proxmoxAdmin.Post("/ha/arm", s.requireStaff(""), s.adminHAArm)
	proxmoxAdmin.Post("/ha/disarm", s.requireStaff(""), s.adminHADisarm)
	proxmoxAdmin.Get("/ha/manager-status", s.requireStaff("infra"), s.adminHAManagerStatus)
	proxmoxAdmin.Get("/cluster/log", s.requireStaff("infra"), s.adminClusterLog)
	proxmoxAdmin.Get("/cluster/tasks", s.requireStaff("infra"), s.adminClusterTasks)
	proxmoxAdmin.Get("/fw-groups", s.requireStaff("infra"), s.adminListFWGroups)
	proxmoxAdmin.Post("/fw-groups", s.requireStaff(""), s.adminCreateFWGroup)
	proxmoxAdmin.Delete("/fw-groups", s.requireStaff(""), s.adminDeleteFWGroup)
	proxmoxAdmin.Get("/fw-groups/:group/rules", s.requireStaff("infra"), s.adminListFWGroupRules)
	proxmoxAdmin.Post("/fw-groups/:group/rules", s.requireStaff(""), s.adminCreateFWGroupRule)
	proxmoxAdmin.Delete("/fw-groups/:group/rules/:pos", s.requireStaff(""), s.adminDeleteFWGroupRule)
	proxmoxAdmin.Get("/firewall-rules", s.requireStaff("infra"), s.adminListClusterFirewallRules)
	proxmoxAdmin.Post("/firewall-rules", s.requireStaff(""), s.adminCreateClusterFirewallRule)
	proxmoxAdmin.Delete("/firewall-rules/:pos", s.requireStaff(""), s.adminDeleteClusterFirewallRule)
	proxmoxAdmin.Get("/fw-aliases", s.requireStaff("infra"), s.adminListFWAliases)
	proxmoxAdmin.Post("/fw-aliases", s.requireStaff(""), s.adminCreateFWAlias)
	proxmoxAdmin.Delete("/fw-aliases/:name", s.requireStaff(""), s.adminDeleteFWAlias)
	proxmoxAdmin.Get("/pools", s.requireStaff("infra"), s.adminListPools)
	proxmoxAdmin.Post("/pools", s.requireStaff(""), s.adminCreatePool)
	proxmoxAdmin.Put("/pools/:pool_id", s.requireStaff(""), s.adminUpdatePool)
	proxmoxAdmin.Delete("/pools/:pool_id", s.requireStaff(""), s.adminDeletePool)
	proxmoxAdmin.Put("/pools/:pool/members", s.requireStaff(""), s.adminPoolUpdateMembers)
	proxmoxAdmin.Put("/pools/:pool_id/members", s.requireStaff(""), s.adminPoolUpdateMembers)
	proxmoxAdmin.Get("/ceph-status", s.requireStaff("infra"), s.adminCephStatus)
	proxmoxAdmin.Get("/ceph/pools/:pool", s.requireStaff("infra"), s.adminCephPoolDetail)
	proxmoxAdmin.Get("/sdn/zones", s.requireStaff("infra"), s.adminSDNZones)
	proxmoxAdmin.Get("/sdn/vnets", s.requireStaff("infra"), s.adminSDNVNets)
	proxmoxAdmin.Get("/cluster-storages", s.requireStaff("infra"), s.adminClusterStoragesList)
	proxmoxAdmin.Post("/cluster-storages", s.requireStaff(""), s.adminClusterStorageCreate)
	proxmoxAdmin.Put("/cluster-storages/:name", s.requireStaff(""), s.adminClusterStorageUpdate)
	proxmoxAdmin.Delete("/cluster-storages/:name", s.requireStaff(""), s.adminClusterStorageDelete)
	proxmoxAdmin.Get("/nodes/:node/prune", s.requireStaff("infra"), s.adminPruneBackupsPreview)
	proxmoxAdmin.Post("/nodes/:node/prune", s.requireStaff(""), s.adminPruneBackups)
	proxmoxAdmin.Get("/nodes/:node/serial-proxy", s.requireStaff("infra"), s.adminNodeSerialProxy)
	proxmoxAdmin.Get("/nodes/:node/report", s.requireStaff("infra"), s.adminNodeReport)
	proxmoxAdmin.Get("/nodes/:node/rrd", s.requireStaff("infra"), s.adminProxmoxNodeRRDData)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/rrd", s.requireStaff("infra"), s.adminProxmoxQemuRRDData)
	proxmoxAdmin.Get("/nodes/:node/qemu", s.requireStaff("infra"), s.adminProxmoxQemuPerNode)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/cloud-init", s.requireStaff("infra"), s.adminProxmoxCloudInitGet)
	proxmoxAdmin.Get("/nodes/:node/dns", s.requireStaff("infra"), s.adminNodeDNSGet)
	...
	proxmoxAdmin.Put("/nodes/:node/time", s.requireStaff(""), s.adminNodeTimeSet)
	proxmoxAdmin.Get("/version", s.requireStaff("infra"), s.adminProxmoxVersion)
	proxmoxAdmin.Get("/next-id", s.requireStaff("infra"), s.adminProxmoxNextID)
	proxmoxAdmin.Get("/access/users", s.requireStaff("infra"), s.adminAccessUsersList)
	proxmoxAdmin.Post("/access/users", s.requireStaff(""), s.adminAccessUserCreate)
	proxmoxAdmin.Put("/access/users/:userid", s.requireStaff(""), s.adminAccessUserUpdate)
	proxmoxAdmin.Delete("/access/users/:userid", s.requireStaff(""), s.adminAccessUserDelete)
	proxmoxAdmin.Get("/access/groups", s.requireStaff("infra"), s.adminAccessGroupsList)
	proxmoxAdmin.Get("/access/roles", s.requireStaff("infra"), s.adminAccessRolesList)
	proxmoxAdmin.Get("/access/acl", s.requireStaff("infra"), s.adminAccessACLList)
	proxmoxAdmin.Get("/templates", s.requireStaff("infra"), s.adminProxmoxTemplates)
	proxmoxAdmin.Delete("/templates/:vmid", s.requireStaff(""), s.adminProxmoxTemplateDelete)
	proxmoxAdmin.Get("/snapshots", s.requireStaff("infra"), s.adminProxmoxSnapshotsList)
	proxmoxAdmin.Post("/snapshots", s.requireStaff(""), s.adminProxmoxSnapshotCreate)
	proxmoxAdmin.Post("/snapshots/rollback", s.requireStaff(""), s.adminProxmoxSnapshotRollback)
	proxmoxAdmin.Delete("/snapshots/:snapname", s.requireStaff(""), s.adminProxmoxSnapshotDelete)
	// QEMU snapshots per node: POST create + GET list + DELETE + rollback (node-scoped)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/snapshot", s.requireStaff("infra"), s.adminProxmoxQemuSnapshotList)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/snapshot", s.requireStaff(""), s.adminProxmoxQemuSnapshotCreate)
	proxmoxAdmin.Delete("/nodes/:node/qemu/:vmid/snapshot/:snapname", s.requireStaff(""), s.adminProxmoxQemuSnapshotDelete)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/snapshot/rollback", s.requireStaff(""), s.adminProxmoxQemuSnapshotRollback)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/snapshot/:snapname/rollback", s.requireStaff(""), s.adminProxmoxQemuSnapshotRollback)
	...
	proxmoxAdmin.Get("/nodes/:node/lxc/:vmid/migrate", s.requireStaff("infra"), s.adminProxmoxLxcMigrateStatus)
	proxmoxAdmin.Post("/nodes/:node/lxc/:vmid/migrate", s.requireStaff(""), s.adminProxmoxLxcMigrate)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/config", s.requireStaff("infra"), s.adminProxmoxQemuConfigGet)
	proxmoxAdmin.Put("/nodes/:node/qemu/:vmid/config", s.requireStaff(""), s.adminProxmoxQemuConfigSet)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/notes", s.requireStaff("infra"), s.adminProxmoxQemuNotesGet)
	proxmoxAdmin.Put("/nodes/:node/qemu/:vmid/notes", s.requireStaff(""), s.adminProxmoxQemuNotesSet)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/tags", s.requireStaff("infra"), s.adminProxmoxQemuTagsGet)
	proxmoxAdmin.Put("/nodes/:node/qemu/:vmid/tags", s.requireStaff(""), s.adminProxmoxQemuTagsSet)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/reset", s.requireStaff(""), s.adminProxmoxQemuReset)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/resume", s.requireStaff(""), s.adminProxmoxQemuResume)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/pause", s.requireStaff("infra"), s.adminProxmoxQemuPauseStatus)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/pause", s.requireStaff(""), s.adminProxmoxQemuPause)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/hibernate", s.requireStaff("infra"), s.adminProxmoxQemuHibernateStatus)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/hibernate", s.requireStaff(""), s.adminProxmoxQemuHibernate)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/migrate", s.requireStaff("infra"), s.adminProxmoxQemuMigrateStatus)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/migrate", s.requireStaff(""), s.adminProxmoxQemuMigrate)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/firewall", s.requireStaff("infra"), s.adminProxmoxQemuFirewallStatus)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/firewall", s.requireStaff(""), s.adminProxmoxQemuFirewallCreate)
	proxmoxAdmin.Delete("/nodes/:node/qemu/:vmid/firewall/:pos", s.requireStaff(""), s.adminProxmoxQemuFirewallDelete)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/firewall/options", s.requireStaff("infra"), s.adminProxmoxQemuFirewallOptionsGet)
	proxmoxAdmin.Put("/nodes/:node/qemu/:vmid/firewall/options", s.requireStaff(""), s.adminProxmoxQemuFirewallOptionsSet)
	// QEMU agent passthrough: GET infra-readable (NOC + platform_admin), POST platform_admin only — proxmox murni via proxmoxAdapterFor.
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/agent", s.requireStaff("infra"), s.adminProxmoxQemuAgent)
	proxmoxAdmin.Get("/nodes/:node/qemu/:vmid/agent/*", s.requireStaff("infra"), s.adminProxmoxQemuAgent)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/agent", s.requireStaff(""), s.adminProxmoxQemuAgent)
	proxmoxAdmin.Post("/nodes/:node/qemu/:vmid/agent/*", s.requireStaff(""), s.adminProxmoxQemuAgent)
	...
	onidelAdmin.Get("/health", s.requireStaff("infra"), s.adminOnidelHealth)
	onidelAdmin.Get("/health/detail", s.requireStaff("infra"), s.adminOnidelHealthDetail)
	onidelAdmin.Get("/instances", s.requireStaff("infra"), s.adminOnidelInstances)
	onidelAdmin.Get("/instances/:instance_id", s.requireStaff("infra"), s.adminOnidelInstanceDetail)
	onidelAdmin.Get("/orders", s.requireStaff("infra"), s.adminOnidelOrders)
	onidelAdmin.Get("/invoices", s.requireStaff("infra"), s.adminOnidelInvoices)
	onidelAdmin.Post("/instances/:instance_id/suspend", s.requireStaff(""), s.adminOnidelInstanceSuspend)
	onidelAdmin.Post("/instances/:instance_id/unsuspend", s.requireStaff(""), s.adminOnidelInstanceUnsuspend)
	onidelAdmin.Post("/instances/:instance_id/terminate", s.requireStaff(""), s.adminOnidelInstanceTerminate)
	onidelAdmin.Post("/regions/sync", s.requireStaff(""), s.adminOnidelRegionsSync)
	onidelAdmin.Post("/catalog/sync", s.requireStaff(""), s.adminOnidelCatalogSync)
	// VMware: /admin/vmware/:id/* — murni vmware (kind==vmware), inventory vs proxmox.
	vmwareAdmin := admin.Group("/vmware/:id")
	vmwareAdmin.Get("/inventory", s.requireStaff("infra"), s.adminVMwareInventory)
	vmwareAdmin.Get("/datastores", s.requireStaff("infra"), s.adminVMwareDatastores)
	vmwareAdmin.Get("/perf", s.requireStaff("infra"), s.adminProviderPerf)
	vmwareAdmin.Get("/perf/:vmid", s.requireStaff("infra"), s.adminVMwarePerfDetail)
	vmwareAdmin.Get("/snapshots", s.requireStaff("infra"), s.adminVmwareSnapshotsList)
	vmwareAdmin.Post("/snapshots", s.requireStaff(""), s.adminVmwareSnapshotCreate)
	vmwareAdmin.Post("/snapshots/revert", s.requireStaff(""), s.adminVmwareSnapshotRevert)
	vmwareAdmin.Post("/snapshots/:snap/revert", s.requireStaff(""), s.adminVmwareSnapshotRevert)
	...
	vmwareAdmin.Get("/clusters/:cluster", s.requireStaff("infra"), s.adminVMwareClusterDetail)
	vmwareAdmin.Get("/pools/:pool", s.requireStaff("infra"), s.adminVMwarePoolDetail)
	vmwareAdmin.Get("/networks", s.requireStaff("infra"), s.adminVMwareNetworks)
	...
	// Dokploy: /admin/dokploy/* — murni dokploy, platform_admin only (tanpa :id, single instance).
	dokployAdmin := admin.Group("/dokploy")
	dokployAdmin.Get("/logs", s.requireStaff("infra"), s.adminDokployLogs)
	dokployAdmin.Get("/projects/:id", s.requireStaff("infra"), s.adminDokployProjectDetail)
	dokployAdmin.Post("/sync", s.requireStaff(""), s.adminDokploySync)
	dokployAdmin.Get("/db/:entity", s.requireStaff(""), s.adminDokployDBList)
	dokployAdmin.Delete("/db/:entity/:remote_id", s.requireStaff(""), s.adminDokployDBDelete)
	...
	admin.Get("/storage-backends/:code", s.requireStaff("infra"), s.handleGetStorageBackend)
	admin.Get("/storage-backends/:code/buckets", s.requireStaff("infra"), s.handleListStorageBackendBuckets)
	...
	// Dokploy universal proxy (/v1/dokploy/*) — provider-prefixed (dokploy-*)
	// stays platform_admin only; allowAudiences(admin,auth) keeps it on
	// admin/auth domains only. The per-provider admin mirror is under
	// /admin/dokploy/* (dokployAdmin group) — universal proxy kept for
	// backward compat until frontend migrates to /admin/dokploy/*.
	v1.All("/dokploy/*", s.authAny(), s.allowAudiences(audienceAdmin, audienceAuth), s.requireStaff("auto"), s.dokployProxy)
}
