// Staff notification center: inbox (GET /notifications, `data: null` when
// empty) with an unread badge, mark-read single/all and per-channel /
// per-event preference switches saved via PATCH /notifications/preferences.
// User-scoped JWT only — no X-Organization-ID.
...
export default function StaffNotificationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      // The backend answers data:null when there is nothing to show.
  ...
  const togglePreference = async (key: keyof Preferences, value: boolean) => {
    ...
    setSavingKey(key)
    // Optimistic flip; the whole preference object is sent so a partial save
    // can never drop unrelated keys.
          ...
          <CardContent className="space-y-2">
            {loading ? (
              ...
              <EmptyState
                ...
                description="Billing and instance events land here."
              />
            ) : (
