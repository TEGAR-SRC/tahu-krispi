...
export default function CreateOnidelVmPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success(`Onidel VM "${name.trim()}" provisioning started`)
      if (data?.id) navigate(`/admin/instances/${data.id}`)
      else navigate(`/admin/onidel/${providerId}/onidel`)
    } catch (cause) {
  ...
  return (
    <ProviderShell
      ...
      description="Provisions an Onidel VM via POST /instances with region routing and service_kind=vm."
    >
      <Card>
        <CardContent className="space-y-4 pt-6">
          <div className="space-y-1.5">
            <Label htmlFor="onidel-name">Name *</Label>
            <Input id="onidel-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="onidel-vm-01" />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-1.5">
              <Label htmlFor="onidel-vcpu">vCPU</Label>
              <Input id="onidel-vcpu" type="number" min={1} value={vcpu} onChange={(e) => setVcpu(Math.max(1, Number(e.target.value) || 1))} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="onidel-ram">RAM (MB)</Label>
              <Input id="onidel-ram" type="number" min={128} step={128} value={ram} onChange={(e) => setRam(Math.max(128, Number(e.target.value) || 128))} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="onidel-disk">Disk (GB)</Label>
              <Input id="onidel-disk" type="number" min={5} value={disk} onChange={(e) => setDisk(Math.max(5, Number(e.target.value) || 5))} />
            </div>
          </div>
