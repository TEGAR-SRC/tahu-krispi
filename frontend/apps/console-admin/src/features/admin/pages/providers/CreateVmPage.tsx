...
export default function CreateVmPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      else navigate(`/admin/proxmox/${providerId}`)
    } catch (cause) {
  ...
  return (
    <ProviderShell
      ...
      title="Create VM"
      description="Provisions a Proxmox VM via POST /instances with region routing and service_kind=vm."
    >
      <Card>
        <CardContent className="space-y-4 pt-6">
          <div className="space-y-1.5">
            <Label htmlFor="vm-name">Name *</Label>
            <Input id="vm-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="vm-01" />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-1.5">
              ...
              <Input id="vm-vcpu" type="number" min={1} value={vcpu} onChange={(e) => setVcpu(Math.max(1, Number(e.target.value) || 1))} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="vm-ram">RAM (MB)</Label>
              <Input id="vm-ram" type="number" min={128} step={128} value={ram} onChange={(e) => setRam(Math.max(128, Number(e.target.value) || 128))} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="vm-disk">Disk (GB)</Label>
              <Input id="vm-disk" type="number" min={5} value={disk} onChange={(e) => setDisk(Math.max(5, Number(e.target.value) || 5))} />
            </div>
          </div>
