...
export default function OnidelCreateChoicesPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    apiGet<ProviderRow[]>("/admin/providers")
      .then(({ data }) => {
        if (cancelled) return
  ...
  const rows: ChoiceRow[] = useMemo(
    () => [
      {
        ...
        kind: "vm",
        title: "Create VM",
        description: "Provision an Onidel VM via POST /instances (service_kind=vm) with region routing and X-Organization-ID.",
      ...
      },
      {
        ...
        description: containerSupported
          ? "Provision an Onidel container via POST /instances (service_kind=container)."
    ...
    [providerId, vmSupported, containerSupported],
  )
  ...
  const columns: SimpleColumn<ChoiceRow>[] = [
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="font-mono text-xs">{r.kind}</Badge>
        </div>
      ),
    },
    {
  ...
  return (
    <ProviderShell
      ...
      title="Create"
      description={
        provider ? `Choose what to create on ${provider.code} (${provider.kind}). VM is available; container is unavailable on Onidel.` : "Choose VM or Container for this Onidel provider."
      }
    >
      ...
      {!isOnidel && !loading ? (
        <Card className="border-amber-200 bg-amber-50/50">
          <CardContent className="pt-6 text-sm text-muted-foreground">
            Provider <span className="font-mono">{providerId.slice(0, 8)}…</span> is kind <span className="font-mono">{provider?.kind}</span>, not <span className="font-mono">onidel</span>. This chooser is intended for Onidel providers only.
          </CardContent>
        </Card>
      ) : null}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className={vmSupported ? "border-primary/30" : ""}>
          <CardHeader>
            ...
            <CardDescription>Onidel VM — billed via vCPU/RAM/disk through POST /instances with service_kind=vm.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">Routes via region_id (regions.provider_id == this provider). Requires X-Organization-ID.</p>
            <div className="flex gap-2">
              <Button onClick={() => navigate(`/admin/onidel/${providerId}/create`)} disabled={!vmSupported}>Create VM</Button>
              <Button variant="outline" asChild>
                <Link to={`/admin/onidel/${providerId}/onidel`}>Back to catalog</Link>
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">Target: <span className="font-mono">POST /v1/instances</span> {"{ name, region_id, service_kind:'vm', cpu, ram, disk }"} + <span className="font-mono">X-Organization-ID</span></p>
          </CardContent>
        </Card>
        ...
        <Card className={!containerSupported ? "opacity-90" : "border-primary/30"}>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button disabled={!containerSupported} onClick={() => navigate(`/admin/onidel/${providerId}/create-container`)}>Create Container</Button>
              <Button variant="outline" asChild>
                <Link to="/admin/providers">All providers</Link>
              </Button>
            </div>
            {!containerSupported ? <p className="text-xs text-amber-600 dark:text-amber-400">If Onidel later supports containers, wire this card to POST /instances with service_kind=container.</p> : null}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Tabular view of the same choices — useful for keyboard navigation and bulk-action parity.</CardDescription>
        </CardHeader>
        <CardContent>
